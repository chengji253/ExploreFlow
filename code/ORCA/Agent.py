...
class Agent:
    ...
    def computeNewVelocity(self, timeStep):
        ...
        for i in range(len(self.obstacleNeighbors_)):
            ...
            # / *无碰撞。计算腿。斜视时，两条腿可以来自一个顶点。非凸顶点时腿延伸截止线。 * /
        ...
        for i in range(len(self.agentNeighbors_)):
            ...
            if distSq > combinedRadiusSq:
                ...
                #     unitW = w / wLength
                # else:
                #     # 处理 wLength 为零的情况，可能是设置 unitW 为一个默认值或忽略
                #     unitW = Vector2(0, 0)
                unitW = w / wLength
                line.direction = Vector2(unitW.y_, -unitW.x_)
        ...
        # if self.newVelocity_ is None:
        #     # print("Error: self.newVelocity_ is None after calling linearProgram2!")
        #     self.newVelocity_ = Vector2(0, 0)
...
"""
* @relates Agent
* @brief 在指定的直线上求解一维线性规划
* 受直线和圆形定义的线性约束约束。
* @param[in] lines 定义线性约束的直线。
* @param[in] lineNo 指定的直线约束。
* @param[in] radius 圆形约束的半径。
* @param[in] optVelocity 优化速度。
* @param[in] directionOpt 如果应优化方向，则为 True。
* @param[in, out] result 对线性规划结果的引用。
* @return 如果成功，则为 True。
"""

# lines optVelocity 是引用 改的值要传出去
# result bool 值需要传出去
# lines optVelocity的值 似乎没有变 都在右边
...
"""
* @relates Agent
* @brief 解决受线性约束（由线和圆形约束定义）约束的二维线性规划。
* @param[in] lines 定义线性约束的线。
* @param[in] radius 圆形约束的半径。
* @param[in] optVelocity 优化速度。
* @param[in] directionOpt 如果应优化方向，则为 True。
* @param[in, out] result 对线性规划结果的引用。
* @return 失败的线数，以及成功时的线数。
"""


# linearProgram2 函数
#  lines optVelocity result 是引用 回传int值
    ...
    for i in range(beginLine, len(lines)):
        ...
        if det(lines[i].direction, lines[i].point - result) > distance:
            ...
            if len_line < len(projLines):
                # 理论上不应该发生。如果失败，是由于小的浮动误差，保持当前结果
                result = tempResult
