...
class KdTree:
    ...
    def buildAgentTree(self):
        active_agents = [agent for agent in self.simulator_.agents_ if agent.agent_exist_ is True]
        self.agents_ = active_agents[:]  # 复制以避免修改原始列表
        num_agents = len(self.agents_)
        max_nodes = 2 * num_agents - 1  # 最大节点数
        self.agentTree_ = [AgentTreeNode() for _ in range(max_nodes)]
        self.buildAgentTreeRecursive(0, num_agents, 0)

    # def buildAgentTree(self):
    #     # if len(self.agents_) < len(self.simulator_.agents_):
    #     #     self.agents_.extend(self.simulator_.agents_[len(self.agents_):])
    #     #     self.agentTree_.resize(2 * len(self.agents_) - 1)
    #     # if self.agents_:
    #     num_agents = len(self.simulator_.agents_)
    #     max_nodes = 2 * num_agents - 1  # 最大节点数
    #     self.agents_ = self.simulator_.agents_
    #     # self.agents_ = copy.deepcopy(self.simulator_.agents_)
    #     # self.agents_ = self.copy_agents()
    #     self.agentTree_ = [AgentTreeNode() for _ in range(max_nodes)]
    #     self.buildAgentTreeRecursive(0, len(self.agents_), 0)

    # def copy_agents(self):
    #     copy_agent_list = []
    #     for idx, agent in self.simulator_.agents_.items():
    #         copy_agent_list.append(copy.deepcopy(agent))
    #     return copy_agent_list
        ...
        if end - begin > RVO_MAX_LEAF_SIZE:
            ...
            while left < right:
                while left < right and \
                        ((isVertical and self.agents_[left].position_.x_ < splitValue) or (not isVertical and self.agents_[left].position_.y_ < splitValue)):
                    left += 1
                while right > left and \
                        ((isVertical and self.agents_[right - 1].position_.x_ >= splitValue) or (not isVertical and self.agents_[right - 1].position_.y_ >= splitValue)):
                    right -= 1
                if left < right:
                ...
                for j in range(len(obstacles)):
                    if i != j:
                        ...
                        if j1LeftOfI >= -RVO_EPSILON and j2LeftOfI >= -RVO_EPSILON:
                            ...
                            leftSize += 1
                            rightSize += 1
                        if (max(leftSize, rightSize), min(leftSize, rightSize)) >= (max(minLeft, minRight), min(minLeft, minRight)):
                            break
                if (max(leftSize, rightSize), min(leftSize, rightSize)) < (max(minLeft, minRight), min(minLeft, minRight)):
                    minLeft = leftSize
                    minRight = rightSize
            ...
            for j in range(len(obstacles)):
                if i != j:
                    ...
                    if j1LeftOfI >= -RVO_EPSILON and j2LeftOfI >= -RVO_EPSILON:
                        ...
                        newObstacle = Obstacle()  # 假设Obstacle类已定义
    ...
    def queryAgentTreeRecursive(self, agent, rangeSq, node):
        if self.agentTree_[node].end - self.agentTree_[node].begin <= RVO_MAX_LEAF_SIZE:
            for i in range(self.agentTree_[node].begin, self.agentTree_[node].end):
                agent.insertAgentNeighbor(self.agents_[i], rangeSq)
        else:
            ...
            if distSqLeft < distSqRight:
                if distSqLeft < rangeSq:
                    self.queryAgentTreeRecursive(agent, rangeSq, self.agentTree_[node].left)
                    if distSqRight < rangeSq:
                        self.queryAgentTreeRecursive(agent, rangeSq, self.agentTree_[node].right)
    ...
    def queryVisibilityRecursive(self, vector1, vector2, radius, node):
        if node:
            ...
            if q1LeftOfI >= 0.0 and q2LeftOfI >= 0.0:
                return self.queryVisibilityRecursive(vector1, vector2, radius, node.left) and ((q1LeftOfI * q1LeftOfI * invLengthI >= radius * radius and q2LeftOfI * q2LeftOfI * invLengthI >= radius * radius) or self.queryVisibilityRecursive(vector1, vector2, radius, node.right))
            if q1LeftOfI <= 0.0 and q2LeftOfI <= 0.0:
                return self.queryVisibilityRecursive(vector1, vector2, radius, node.right) and ((q1LeftOfI * q1LeftOfI * invLengthI >= radius * radius and q2LeftOfI * q2LeftOfI * invLengthI >= radius * radius) or self.queryVisibilityRecursive(vector1, vector2, radius, node.left))
            if q1LeftOfI >= 0.0 and q2LeftOfI <= 0.0:
                return self.queryVisibilityRecursive(vector1, vector2, radius, node.left) and self.queryVisibilityRecursive(vector1, vector2, radius, node.right)
            ...
            return point1LeftOfQ * point2LeftOfQ >= 0.0 and point1LeftOfQ * point1LeftOfQ * invLengthQ > radius * radius and point2LeftOfQ * point2LeftOfQ * invLengthQ > radius * radius and self.queryVisibilityRecursive(vector1, vector2, radius, node.left) and self.queryVisibilityRecursive(vector1, vector2, radius, node.right)
        return True