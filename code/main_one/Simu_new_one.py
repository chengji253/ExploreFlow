...
class Simu_main:
    ...
    def init_simulation_Astar(self):
        ...
        self.swarm.v_max = [4.5 for i in range(len(self.swarm.pos_all))]
    ...
    def init_simulation_sipp(self):
        self.title_name = self.title_name + "sipp"
        ...
        self.swarm.set_start_goal_node(self.mapInfo.start_idx, self.mapInfo.end_idx, 0, self.mapInfo.number_cells - 1)
        # self.mapInfo.draw_mapInfo()
        # self.ecbs_planner_run()
        self.sipp_planner_run()
        t4 = time.time()

        logging.info("mapInfo_time=" + str(t2 - t1) + "s")
        logging.info("mapCo_time=" + str(t3 - t2) + "s")
        # logging.info("ecbs_time=" + str(t4 - t3) + "s")

    def init_simulation_ecbs(self):
        self.title_name = self.title_name + "ecbs"
    ...
    def deal_ecbs_path(self, path_pos):
        ...
        all_coords = path_pos.copy()
        # 遍历列表并删除 y 坐标大于 42 的元素
        for i in range(len(all_coords)):
            # 过滤掉 y 坐标大于  的元素
            all_coords[i] = [coord for coord in all_coords[i] if coord[1] <= 45]
            all_coords[i].append(goal_list[i])
        return all_coords

    def ecbs_planner_run(self):
        path_pos = []
        self.ecbs_path = self.read_yaml_file('../path_res/ecbs.yaml')
        # self.ecbs_path = self.read_yaml_file('path_res/p_sipp.yaml')
        for _ in range(10):
            for sub_list in self.ecbs_path:
                path_pos.append(sub_list.copy())

        path_pos = self.deal_ecbs_path(path_pos)
        self.swarm.des_path_pos = path_pos
        self.swarm.para_set_planner_Other_planner()

    def sipp_planner_run(self):
        path_pos = []
        self.sipp_path = self.read_yaml_file('../path_res/p_sipp.yaml')

        for _ in range(10):
            for sub_list in self.sipp_path:
                path_pos.append(sub_list.copy())
    ...
    def read_yaml_file(self, file_name):
        with open(file_name, 'r', encoding='utf-8') as f:
            ...
            for i in range(10):
                agent_key = f'agent{i}'
                agent_path = data['schedule'].get(agent_key)
                if agent_path:
                    path_list = []
                    for item in agent_path:
                        sub_list = [item['x'], item['y']]
                        path_list.append(sub_list)
                    all_agents_paths.append(path_list)
            return all_agents_paths
    ...
    def simulate_main_flow_planner(self):
        ...
        while t * step < total_time:
            ...
            if t % 20 == 0:
                ...
                # plotPic.visualize_traj_only_path(ws_model, self.swarm, self.mapCo.boundary, self.mapInfo, time=t * step,
                #                                name=name_n % str(t / 10))
...
def run_flow_planner(map_name, num, forest_bool):
    ...
    s1.init_simulation_flow_planner()
...
def run_ecbs_planner(map_name, num, forest_bool):
    logging.info("run_ecbs_planner")
    # 如何更好地解决死锁的问题
    # 周围没有机器人 然后RVO还是计算0 此时就进行移动
    # 如果周围有机器人 计算是0是确定的 可能是为了避碰
    time1 = time.time()
...
def run_sipp_planner(map_name, num, forest_bool):
    logging.info("run_sipp_planner")
    # 如何更好地解决死锁的问题
    # 周围没有机器人 然后RVO还是计算0 此时就进行移动
    # 如果周围有机器人 计算是0是确定的 可能是为了避碰
    time1 = time.time()
...
def run_number_all_forest():
    ...
    for map_name in forest_name_list:
        for swarm_num in num_list:
            ...
            # run_ecbs_planner(map_name, swarm_num, True)
            # run_sipp_planner(map_name, swarm_num, True)
